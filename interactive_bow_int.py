...
def main():
    B_half = cfg.BOA / 2.0
    D = cfg.DOA
    config_key = "Bow_Intermediate_Points"
    ...
    if config_key in data:
        ...
        # Default 7-point bezier (already dimensioned)
        pts = [[0.0, 0.0], [B_half*0.1, D*0.1], [B_half*0.2, D*0.3], [B_half*0.3, D*0.5], [B_half*0.4, D*0.7], [B_half*0.5, D*0.85], [B_half*0.6, D]]
    ...
    ax.set_ylim(-1, D + 1)
    ax.set_aspect('equal')
    ax.set_title(f'Interactieve Bow Intermediate Rhino-Style Spline (7 Punten)\nLocatie: {cfg.BOW_INT_PCT}%.\nSleep de stippen!')
    ax.set_xlabel('Breedte (y) - Midden zit rechts')
    ax.set_ylabel('Hoogte (z) - Vanaf de kiel')
    ax.grid(True, alpha=0.3)
    
    custom_lines = [Line2D([0], [0], color='b', lw=3),
                    Line2D([0], [0], color='r', ls='--', marker='o')]
    ax.legend(custom_lines, ['Rhino-Style Curve', 'Controle Polygon (7 punten)'], loc='lower left')
